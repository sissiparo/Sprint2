...
//<<<<<<< HEAD
 /* public List<BaseData> userStory7(Date startDate, Date endDate){
	...
	SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
    	...
    	Query q = em.createQuery("SELECT baseDate,imsi FROM BaseData  where baseDate between :startDate '' and :endDate '' order by imsi")
...
//<<<<<<< HEAD
 ...
 public List<BaseData> userStory9(String startDate, String endDate){
 	...
 	Query q = em.createQuery("select baseDate, imsi, eventcause.eventID, eventcause.causeCode,eventcause.causeDescription from BaseData where BaseDate >= :startDate and BaseDate <= :endDate")
 	.setParameter("startDate", calStart, TemporalType.DATE)
     .setParameter("endDate", calEnd, TemporalType.DATE);
 	List<BaseData> listOfBaseData = q.getResultList();
		return listOfBaseData;
