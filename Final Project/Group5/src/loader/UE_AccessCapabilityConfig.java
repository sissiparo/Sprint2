		...
		for (int i = 1; i < currentSheet.getRows(); i++) {
			...
			if (row.length > 0) {
				...
				for (int j = 0; j < indivCapabilities.length; j++) {
					...
					UE_AccessCapability ueac = new UE_AccessCapability(ue_acc);
					em.persist(ueac);
					}
				}
			}
...
q.setParameter("accessCapability", accessCapability);
    ...
    public UE_AccessCapability getUEAccessCapability(int id, String tac){
    	UeAccesscapabilityPK ueacpk = new UeAccesscapabilityPK(tac, id);
    	Query q = em.createQuery("select o from UE_AccessCapability o where id=:id");
q.setParameter("id", ueacpk);
    	 List<UE_AccessCapability> listOfAccessCapability = q.getResultList();
    	 if (listOfAccessCapability.size() == 0)
 			return null;
 		else
 			return listOfAccessCapability.get(0);
